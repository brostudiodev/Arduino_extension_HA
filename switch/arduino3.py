...
from custom_components import arduino3
"""
...
class ArduinoSwitch(SwitchDevice):
    ...
    def __init__(self, pin, options):
        ...
        if options.get(CONF_NEGATE):
            self.turn_on_handler = arduino3.BOARD.set_digital_out_low
            self.turn_off_handler = arduino3.BOARD.set_digital_out_high
        else:
            self.turn_on_handler = arduino3.BOARD.set_digital_out_high
            self.turn_off_handler = arduino3.BOARD.set_digital_out_low

        arduino3.BOARD.set_mode(self._pin, self.direction, self.pin_type)

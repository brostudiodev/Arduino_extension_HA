...
from custom_components import arduino2
"""
...
class ArduinoSwitch(SwitchDevice):
    ...
    def __init__(self, pin, options):
        ...
        if options.get(CONF_NEGATE):
            self.turn_on_handler = arduino2.BOARD.set_digital_out_low
            self.turn_off_handler = arduino2.BOARD.set_digital_out_high
        else:
            self.turn_on_handler = arduino2.BOARD.set_digital_out_high
            self.turn_off_handler = arduino2.BOARD.set_digital_out_low

        arduino2.BOARD.set_mode(self._pin, self.direction, self.pin_type)
